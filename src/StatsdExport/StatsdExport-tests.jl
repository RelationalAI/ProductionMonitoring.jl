...
Base.@kwdef mutable struct MockStatsdBackend <: StatsdExport.AbstractServiceBackend
    messages::Set{String} = Set{String}()
end
StatsdExport.send(b::MockStatsdBackend, msg::String) = push!(b.messages, msg)
...
function test_friendly_export_to_statsd(statsd::MockStatsdBackend, exporter::StatsdExporter)
    ...
    # before and after.
    sleep(0.01)
    StatsdExport.send_metric_updates(exporter)
    sleep(0.01)
end

@testset "Counter changes" begin
...
@testset "Changes with labels" begin
    mock_statsd = MockStatsdBackend()
    r = MetricRegistry()
    reqs = register!(r, Counter(;labels=[:action, :response_code]), "requests")
...
@testset "Two metrics exported" begin
    mock_statsd = MockStatsdBackend()
    r = MetricRegistry()
...
@testset "StatsdExporter with default registry" begin
...
@testset "StatsdExporter spawns background thread" begin
    mock_statsd = MockStatsdBackend()
    r = MetricRegistry()
    c = register!(r, Counter(), "cnt")
...
@testset "StatsdExporter not exporting with send_interval=0" begin
    mock_statsd = MockStatsdBackend()
    exporter = StatsdExporter(
...
@testset "StatsdExporter sends UDP packets" begin
    # Create UDP listener and then send packet to it using send(::StatsdBackend, msg)
    server = UDPSocket()
    bind(server, ip"127.0.0.1", 12346)

    client = StatsdExport.UDPBackend(ip"127.0.0.1", 12346)
    StatsdExport.send(client, "test message")
